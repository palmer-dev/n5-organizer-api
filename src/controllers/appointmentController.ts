...
const add = async (
    ...
    const agenda = await models.agenda.find(new Types.ObjectId(req.params.id));
    ...
    const appointment = matchedData<AppointmentCreateBody>(req);

    const {name, notes, startDate, endDate, users} = appointment;

    const agendas = [agenda, ...await models.agenda.findForUsers([
        ...users,
    ])];
...
const search = async (req: Request, res: Response) => {
    const searchQuery = matchedData<AppointmentAvailabilitySearch>(req);

    try {
        const existingAppointments = await models.forUser(new Types.ObjectId(req.user.id))
            .appointment
            .search(
                searchQuery
            );
